...
class Spin_CustomImageDataset(Dataset):
    def __init__(self, img_dir, transform=None, target_transform=None, standarization=False, sidelen = 200, num_octaves=4):
        self.particles = []
        self.img_dir = img_dir
        self.transform = transform
        self.target_transform = target_transform
        ...
        for imname in self.img_names:
            ...
            image = self.gaussian_filters(image)     #ADDED GAUSSIAN FILTERS
...
def get_spin_dataloader(paths_to_data, batch_size, standarization=False):
    """Loads datasets from the provided list of paths.
    ...
    # Get dataset
    particle_dset = Spin_CustomImageDataset(
        img_dir=selected_path,
        transform=transform,
        standarization=standarization
